...
def start(message):
    bot.send_message(message.chat.id, '👋🏻 Приветствую! Это Компьютерщик.\nЕсли у вас есть какой-либо вопрос или проблема - нажмите на кнопку <b>Написать запрос</b> и наши сотрудники в скором времени тебе ответят!', parse_mode='html', reply_markup=markup.markup_main())
